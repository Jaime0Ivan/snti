...
@section('content')
    ...
    <div class='titulo'>
        ...
        <a href="{{ url('pie/create') }}" class='btn btn-success'>Modificar pie de pagina</a>
            ...
            <tbody>
                @foreach($pies as $pie)
                    <tr>
                        <td>{{ $pie->id }}</td>
                        <td>
                        <img class='img-thumbnail img-fluid' src="{{ url(str_replace('public/', '', 'storage/'.$pie->LogoP)) }}" width="100">
                        </td>
                        <td>{{ $pie->TituloT}}</td>
                        <td>{{ $pie->Texto }}</td>
                        <td>
                        <a href="{{url('pie/' .$pie->id. '/edit')}}" class='btn btn-warning'>
                            Editar
                        </a> 
                            <form action="{{ url('pie/'.$pie->id) }}" class='d-inline' method="post">
                                @csrf
                                {{ method_field('DELETE') }}
